                  ...
                  {selectedMaterial.ofertas.map((oferta: any, index: number) => (
                    <div key={index} className="flex items-center justify-between p-4 bg-gray-50 rounded-lg">
                      ...
                      <div className="text-right flex items-center space-x-2">
                        ...
                        {selectedMaterial.usuario?.telefono && (
                          <button
                            onClick={() => {
                              const message = `Hola ${selectedMaterial.usuario.name}, me interesa el material "${selectedMaterial.nombre}". ¿Podemos conversar sobre las ofertas disponibles?`;
                              window.open(`https://wa.me/595${selectedMaterial.usuario.telefono.replace(/\D/g, '')}?text=${encodeURIComponent(message)}`, '_blank');
                            }}
